...
app.delete('/api/terrain/:terrainId', async (req, res) => {
  try {
    ...
    await prisma.objects.deleteMany({ where: { terrainId } });
...
app.delete('/api/road/:roadId', async (req, res) => {
  try {
    ...
    // Delete from DB (파일 삭제 실패해도 DB는 삭제)
    await prisma.objects.deleteMany({ where: { roadId } });
...
app.post('/api/road', async (req, res) => {
  try {
    ...
    // Progress 추적 등록
    progressService.registerJob(dbJob.id, ['road_generation', 'preview_render']);

    // 백그라운드 작업 시작 (비동기)
    (async () => {
      try {
        progressService.updateJobStatus(dbJob.id, 'processing');
        progressService.startStep(dbJob.id, 'road_generation', 'Generating road mesh...');

        const outputPath = path.join(outputDir, `${dbJob.id}.blend`);
        const previewPath = path.join(outputDir, `${dbJob.id}_preview.png`);

        console.log(`[Background] Creating road with ${controlPoints.length} points`);

        // Road 생성
        const scriptPath = path.join(__dirname, 'blender-scripts', 'road_generator_logged.py');
        const paramsFilePath = path.join(outputDir, `${dbJob.id}_params.json`);
        ...
        // Progress 업데이트
        progressService.completeStep(dbJob.id, 'road_generation', 'Road mesh created');
        progressService.startStep(dbJob.id, 'preview_render', 'Rendering preview...');
        progressService.completeStep(dbJob.id, 'preview_render', 'Preview rendered');
        ...
        progressService.completeJob(dbJob.id, { blendFile: outputPath, preview: previewPath });
