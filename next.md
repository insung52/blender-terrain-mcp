terrain 생성할때 지금은 claude api 결과 나오고 나서 progress 팝업창 뜨는데, 그냥 generate terrain 버튼 누르자마자 progress 나오게 변경

멀티 사용자 가능하게?

바이옴 맵 이미지 생성되면 progress 에서 볼 수 있게 하기. (이미지 다? 아니면 하나만?) 온도랑 습도, 높이 3개만 보여주면 좋을듯?

지형에 세밀한 텍스쳐(잔디, 암석 등등) 적용
...
가끔 삭제 실패하는경우 있음. 내가 output 폴더에 있는거 지워버렸다거나. 차피 원본 파일이 없다는 뜻이니까, 삭제 실패하면 그냥 데이터베이스에서 삭제해서 웹페이지에서 더이상 안보이게 수정

정리 : 어떤 파일 ex. road 면 그 road 관련된 blend, log, 데이터베이스 등등 관련된거 삭제. 하나 실패하더라도 나머지는 삭제되게 try문? 같은거로 처리.
