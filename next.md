...
- 최종 목표 : ground color 이미지 형태 변경. 높이, 온도, 습도 등 여러 변수를 통해 땅 색을 지정할 수 있게 결정. ground color 이미지는 x축 온도, y축 습도 이런 형식으로 땅 색을 표현할 수 있도록 함. (근데 이건 ai 기반 지형 생성 개념이랑은 어긋나는 방식인거 같음)
